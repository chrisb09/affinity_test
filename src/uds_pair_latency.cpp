...
/*
 * affinity_test/src/uds_pair_latency.cpp
 * ======================================
 * Unix Domain Socket (UDS) rank-to-rank latency and bandwidth benchmark.
 *
 * Designed for node-local IPC performance characterization (e.g. SmartSim
 * colocated deployment mode with Redis UDS socket communication).
 *
 * Protocol & Methodology:
 *   - Small payload (default 64 B)  ping-pong: half-RTT latency (min, mean, median, max).
 *   - Large payload (default 4 MiB) ping-pong: bidirectional goodput (2 * large_bytes / RTT, GiB/s).
 *   - 10 warmup + 50 timed iterations per pair.
 *   - Evaluates all N*(N-1)/2 node-local rank pairs sequentially.
 *   - Uses distinct unix domain socket paths in /tmp (e.g. /tmp/uds_pair_<jobid>_<r1>_<r2>.sock).
 */
...
static void print_usage(const char* prog) {
    std::cout << "Usage: " << prog << " [options]\n"
              << "  -o, --output <file>        CSV output (default: uds_pair_latency.csv)\n"
...
static PairStats measure_uds_pair(
    ...
    std::vector<char>& sl, std::vector<char>& rl)
{
    ...
    // 1. Small payload ping-pong (Latency)
    for (int w = 0; w < warmup; ++w) {
        if (my_rank == ri) {
            send_all(conn_fd, ss.data(), small_bytes);
    ...
    std::vector<double> half_rtts(iters);
    for (int it = 0; it < iters; ++it) {
        if (my_rank == ri) {
            ...
            double rtt_us = std::chrono::duration<double, std::micro>(b - a).count();
            half_rtts[it] = rtt_us * 0.5;
        } else {
            recv_all(conn_fd, rs.data(), small_bytes);
            send_all(conn_fd, ss.data(), small_bytes);
        }
    ...
    // 2. Large payload ping-pong (Bandwidth)
    for (int w = 0; w < warmup; ++w) {
        if (my_rank == ri) {
            send_all(conn_fd, sl.data(), large_bytes);
    ...
    for (int it = 0; it < iters; ++it) {
        if (my_rank == ri) {
            ...
            recv_all(conn_fd, rl.data(), large_bytes);
            auto b = std::chrono::high_resolution_clock::now();
            double rtt_sec = std::chrono::duration<double>(b - a).count();
            bws[it] = (2.0 * static_cast<double>(large_bytes) / GiB) / rtt_sec;
        } else {
            recv_all(conn_fd, rl.data(), large_bytes);
            send_all(conn_fd, sl.data(), large_bytes);
        }
    ...
    if (my_rank == ri) {
        std::sort(half_rtts.begin(), half_rtts.end());
        stats.lat_min_us     = half_rtts.front();
        stats.lat_max_us     = half_rtts.back();
        stats.lat_mean_us    = mean_of(half_rtts);
        stats.lat_median_us  = median_of(half_rtts);
...
int main(int argc, char** argv) {
    ...
    std::string output_file = "uds_pair_latency.csv";
    ...
    // Verify all ranks are strictly node-local (single node execution)
    ...
    if (rank == 0) {
        ...
        std::cout << "=== Precision UDS Pairwise Latency & Bandwidth Benchmark ===\n"
    ...
    for (int i = 0; i < size; ++i) {
        for (int j = i + 1; j < size; ++j) {
            ...
            sp << socket_dir << "/uds_p_" << i << "_" << j << ".sock";
            std::string sock_path = sp.str();

            PairStats stats;
            if (rank == i || rank == j) {
                stats = measure_uds_pair(rank, i, j, sock_path, small_bytes, large_bytes, warmup, iters, ss, rs, sl, rl);
            }
            ...
            if (rank == 0) {
                ...
                if (pair_idx % 200 == 0 || pair_idx == total_pairs) {
                    std::cout << "Progress: [" << pair_idx << "/" << total_pairs << "] pairs completed ("
    ...
    if (rank == 0) {
        csv.close();
        std::cout << "\n[Success] UDS Pairwise Benchmark completed successfully. Saved to '" << output_file << "'.\n";
    }
